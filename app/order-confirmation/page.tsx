...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    // Making the update asynchronous to avoid cascading renders warning in React 19
    const handle = requestAnimationFrame(() => {
      setOrderNumber("EMS-" + Math.floor(100000 + Math.random() * 900000));
    });
        ...
        <div className="bg-card border border-border/50 rounded-[2.5rem] p-8 md:p-12 mb-12 shadow-xl relative overflow-hidden group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left mb-8 border-b border-border pb-8">
            <div>
              ...
              <span className="text-xl font-bold text-pure-green">{orderNumber}</span>
            </div>
            <div>
          ...
          <div className="flex flex-col gap-4">
             <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-2xl border border-border">
                ...
                <div className="text-left">
                   ...
                   <p className="text-xs text-muted-foreground">You will receive an SMS with tracking details once shipped.</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-6">
          <Link href="/orders">
            <Button size="lg" variant="outline" className="px-10 py-8 rounded-2xl font-bold text-lg border-border">
              <Package className="mr-2 w-5 h-5" /> View Order Status
            </Button>
          </Link>
          <Link href="/products">
